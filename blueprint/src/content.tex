...
= &\min\Bigg\{\min\left\{h(M,x) \mid x \in \mathbb{F}_q^n, \left\lfloor \frac{n+1}{2} \right\rfloor < w_h(x) \leq n, w_h(Mx) \leq \left\lfloor \frac{n+1}{2} \right\rfloor\right\},\\
...
In Step \ref{lem:high_weight_partition}, the proof takes the minimum over a set of vectors. In order to take the minimum, we need to know that the set is nonempty. In this case, the set
\[
...
\]
which is empty due to the contradictory conditions $w_h(x) \leq \left \lfloor \frac{n+1}{2} \right\rfloor < w_h(x)$. This error is represented by the \texttt{sorry} on line 1606 in \lean{branchnumber_equiv}. 