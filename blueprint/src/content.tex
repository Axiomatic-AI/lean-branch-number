...
In the formalization of Step \ref{lem:high_weight_partition}, the proof takes the minimum over a set of vectors. To take this minimum, we need to show that the set is nonempty. However, the set
\[
...
\]
can be empty for some matrices. Specifically, when $M = I$ (the identity matrix), we have $w_h(Mx) = w_h(Ix) = w_h(x)$, so the set becomes
\[
...
\]
which is empty due to the contradictory conditions $w_h(x) \leq \left \lfloor \frac{n+1}{2} \right\rfloor < w_h(x)$. This issue corresponds to the \texttt{sorry} on line 1606 in \lean{Branchnumber_efficient}. 

Despite the issue in this intermediate step, the final result in Theorem~\ref{thm:branch_number_main} remains valid. 